    ...
    mounted(containerEl) {
        ...
        this.myp5 = new p5((sketch) => {
            ...
            sketch.draw = async () => {
                ...
                const { isPlaying, freqData, freqBinCount, leftFreqData, sampleRate, analyser, spectrumColor, stroke } = spectrumParams
                ...
                const data = transformFrequencyData(leftFreqData)
                const dataLen = data.length - 50
                for (let i = 0; i < dataLen; i++) {
                    //旋转角度
                    const rTheta = sketch.map(i, 0, dataLen - 1, -sketch.PI,  sketch.PI)  
                    sketch.push()
                    sketch.rotate(rTheta)
                    const height = data[i] / 256 * 56
                    sketch.line(0, r + 10, 0, r + 10 + height)
                    sketch.pop()
                }
                albumTheta += 0.01
            }
...
export const activate = () => {
  ...
  console.log('[ PLUGIN - Activated ] 频谱 - 动感音阶')
}
...
export const deactivate = () => {
  ...
  console.log('[ PLUGIN - Deactivated ] 频谱 - 动感音阶')
}
