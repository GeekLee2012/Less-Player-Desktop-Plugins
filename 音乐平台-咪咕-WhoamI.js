...
const { Category, Playlist, Track, Album, } = common
const { toTrimString, getImageUrlByQuality } = utils
const { randomTextDefault, sha1, aesEncryptDefault, rsaEncryptDefault } = crypto
const { getJson, } = nets
const { registerPlatform, addRequestHandler, getUserAgent } = permissions 
...
export const activate = async (plugin) => {
  ...
  //获取UserAgent
  userAgent = await getUserAgent(plugin)
  //设置Cookie
  setupMiguCookieId()
