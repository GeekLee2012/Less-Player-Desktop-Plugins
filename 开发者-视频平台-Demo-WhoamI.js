...
/* 默认提供的插件API */
const { 
    common, utils, crypto, events, nets, permissions 
} = lessAPI
const { 
    Category, Playlist, Track, Album, Lyric 
} = common
const { 
    isBlank, toTrimString, nextInt, toMillis, toYmd, 
    getImageUrlByQuality, escapeHtml, transformUrl, 
} = utils
const { 
    md5, randomText, rsaEncrypt, aesDecryptText, aesEncryptDefault, aesEncryptHexText, 
} = crypto
const { APIEvents, register, unregister } = events
const { getDoc, getJson, postJson, } = nets
const { registerPlatform, getCookie, addRequestHandler } = permissions
...
const setupCookie = (plugin) => {
    const cookie = getCookie(plugin, 'https://www.demo.com', true)
    //其他处理逻辑
...
const appendRequestHandler = (options) => {
    if(!options) return 
    updateRequstHandler(cachePlugin, {
        id: Demo.CODE,
        appendMode: true,
        ...options
    })
