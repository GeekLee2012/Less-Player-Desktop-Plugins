...
let count = 0, lastRandIndex = 0
const transformFrequencyData = (freqData) => {
    const result = []
    const limit = 10
    const data = Array.from(freqData).slice(limit)

    if (++count % 60 == 0) lastRandIndex = nextInt(3)
    const { accTimes, size, step } = [{
            accTimes: 3,
            size: 20,
            step: 8
        }, {
    ...
    for (let i = 0; i < accTimes; i++) {
        result.push(...result.slice().reverse())
    }
    return result
}
    ...
    mounted(containerEl) {
        ...
        this.myp5 = new p5((sketch) => {
            ...
            sketch.draw = async () => {
                ...
                const maxHeight = 56, baseHeight = 100
                ...
                for (let i = 0; i < dataLen; i++) {
                    ...
                    const height = (Math.abs(data[i]) - baseHeight) / (255 - baseHeight) * maxHeight
...
export const activate = () => {
  ...
  console.log('[ PLUGIN - Activated ] 频谱 - 动感音阶（随机衍变版）')
}
...
export const deactivate = () => {
  ...
  console.log('[ PLUGIN - Deactivated ] 频谱 - 动感音阶（随机衍变版）')
}
