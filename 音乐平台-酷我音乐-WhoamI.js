...
const { 
    toTrimString, toLowerCaseTrimString, toUpperCaseTrimString, 
    toMMssSSS, getImageUrlByQuality,  
} = utils
const { randomTextDefault, md5, sha1 } = crypto
const { getDoc, getJson } = nets
const { registerPlatform, addRequestHandler, getCookie } = permissions
...
const parseCookie = (plugin) => {
    ...
    items.forEach(item => {
        ...
        const key = toTrimString(kvPair[0])
        const value = toTrimString(kvPair[1])
        Object.assign(kwCookie, { [key]: value })
    })
    return kwCookie
}

const setupCookie = async (plugin) => {
    const kwCookie = parseCookie(plugin) || await getCookie(plugin, 'https://www.kuwo.cn/')
        ...
        return new Promise((resolve, reject) => {
            ...
            getJson(url, null, CONFIG).then(json => {
                try {
                    ...
                    //Object.assign(result, { url, cover: getCoverByQuality(cover) })
                } catch(error) {
                    console.log(error)
                }
                resolve(result)
...
export const activate = async (plugin) => {
  ...
  const { cookie, cross, secret } = await setupCookie(plugin)
